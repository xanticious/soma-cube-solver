...
import { createNotationConverter } from './components/NotationConverter/NotationConverter';
  ...
  switch (state) {
    case 'home':
      currentView = createHomePage(app, {
        ...
        onGoToConverter() {
          actor.send({ type: 'GO_TO_CONVERTER' });
        },
      });
    ...
    case 'converter':
      currentView = createNotationConverter(app, {
        onGoHome() {
          actor.send({ type: 'GO_HOME' });
        },
      });

      updateUrl('converter', null);
...
function updateUrl(state: string, notation: string | null) {
  ...
  if (state === 'converter') {
    window.history.pushState(null, '', '#view=converter');
    return;
  }
